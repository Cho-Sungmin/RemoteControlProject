...
LRESULT CRemoteControlView::OnDisplayImg(WPARAM wParam, LPARAM lParam)
{
	CRect rc;
	CImage img;
	int result;
	...
	this->GetClientRect(&rc);
	//WaitForSingleObject(CMainFrame::h_mutex, INFINITE);
	result = img.Load(_T("img.png"));
	//ReleaseMutex(CMainFrame::h_mutex);
	if (result == 0)
	{
		//if(hdc!=NULL) ::ReleaseDC(NULL, hdc);
		img.Draw(hdc, 0, 0, rc.right, rc.bottom-55);
	}
