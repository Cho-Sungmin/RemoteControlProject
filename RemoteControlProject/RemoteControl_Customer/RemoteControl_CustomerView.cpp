...
void CRemoteControlCustomerView::OnInitialUpdate()
{
	...
	/* set unique id ( now, just for a test ) */
...
{
	/* replace controls after connection */
	delCtrl();
...
{
	/* set view includes controls for connection */
	CRect deviceSize;
	CRect rc;
	int width, height;
	CString strID, strPW;
	...
	m_bt_p2p->SetCheck(true);	//default mode : P2P
}
...
{
	/* send msg to engage connection on MainFrame */
